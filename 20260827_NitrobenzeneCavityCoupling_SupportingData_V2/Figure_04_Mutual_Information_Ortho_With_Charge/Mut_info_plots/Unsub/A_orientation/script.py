...
# geometry plus pz orbital numbers
geom_orig = np.loadtxt("../"+sys.argv[1],usecols=(1,2,4))

# mutual information
mut_info_orig = np.loadtxt("../"+sys.argv[2])

# nalpha
#nalpha_orig = np.loadtxt("../"+sys.argv[3])
    ...
    for j in range(i + 1, len(geom)):
        ...
        if d < 1.9:
            if int(geom[i][2]) == 0 or int(geom[j][2])==0:
                ...
                x, y = [geom[i][0], geom[j][0]], [geom[i][1], geom[j][1]]
                mi = mut_info[int(geom[i][2])-1][int(geom[j][2])-1]
                mi_diff = (mut_info[int(geom[i][2])-1][int(geom[j][2])-1]-mut_info_orig[int(geom_orig[i][2])-1][int(geom_orig[j][2])-1])
                mi_rel=(mi_diff/mut_info_orig[int(geom_orig[i][2])-1][int(geom_orig[j][2])-1])*100
                color = cmap(mi/mi_max)
                pl.plot(x, y, color=color, lw=3)
                pl.text(np.average(x), np.average(y), f"{mi_rel:+2.0f} %",va="center",ha="center", size = "small",bbox=dict(facecolor='white',edgecolor='none'))
...
#        else:
#            dens = 2*nalpha[int(geom[4])-1]
#            dens_orig = 2*nalpha_orig[int(geom[4])-1]
#            dens_rel = (dens-dens_orig)/dens_orig
#            pl.text(float(geom[1]),float(geom[2]),f"{dens_rel:+2.0f} %",va="center",ha="center", size = "small",bbox=dict(boxstyle=f"circle,pad={0.3}",facecolor='white',edgecolor='grey')) 
...
#pl.axis('off')
#pl.show()
pl.savefig("Plot_unsubst_diff_y.pdf")
